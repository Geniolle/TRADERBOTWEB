...
function getSimpleStructureContext(value: unknown): SimpleTextContext {
  ...
  if (
    ...
    return {
      label: "Alta",
      situation: "Estrutura favorÃ¡vel para subida",
    };
  }
  ...
  if (
    ...
    return {
      label: "Queda",
      situation: "Estrutura favorÃ¡vel para descida",
    };
  }
  ...
  if (raw.includes("range") || raw.includes("lateral") || raw.includes("sideways")) {
    return {
      label: "Lateral",
      situation: "Mercado sem direÃ§Ã£o clara",
    };
  }
...
function getSimpleEmaAlignmentContext(value: unknown): SimpleTextContext {
  ...
  if (
    ...
    return {
      label: "Alta",
      situation: "MÃ©dias alinhadas para subida",
    };
  }
  ...
  if (
    ...
    return {
      label: "Queda",
      situation: "MÃ©dias alinhadas para descida",
    };
  }
  ...
  if (raw.includes("mixed") || raw.includes("neutral") || raw.includes("misto")) {
    return {
      label: "Misto",
      situation: "MÃ©dias sem alinhamento limpo",
    };
  }
...
function getSimpleSlopeContext(value: unknown): SimpleTextContext {
  ...
  if (
    ...
    return {
      label: "A subir",
      situation: "TendÃªncia a ganhar forÃ§a",
    };
  }
  ...
  if (
    ...
    return {
      label: "A descer",
      situation: "TendÃªncia a perder forÃ§a ou cair",
    };
  }
  ...
  if (raw.includes("flat") || raw.includes("neutral") || raw.includes("lateral")) {
    return {
      label: "Lateral",
      situation: "Sem aceleraÃ§Ã£o clara",
    };
  }
  ...
  return {
    label: "Indefinida",
    situation: "InclinaÃ§Ã£o sem clareza",
  };
}
...
function getSimplePriceVsEmaContext(value: unknown): SimpleTextContext {
  ...
  if (raw.includes("above") || raw.includes("acima") || raw.includes("over")) {
    return {
      label: "Acima",
      situation: "PreÃ§o do lado comprador",
    };
  }
  ...
  if (raw.includes("below") || raw.includes("abaixo") || raw.includes("under")) {
    return {
      label: "Abaixo",
      situation: "PreÃ§o do lado vendedor",
    };
  }

  return {
    label: "Neutro",
    situation: "PreÃ§o sem posiÃ§Ã£o clara",
  };
}
...
function getSimpleDistanceContext(
  ...
  if (numericValue < 0.25) {
    return {
      ...
      situation:
        kind === "support"
          ? "Pouco espaÃ§o atÃ© ao suporte"
          : "Pouco espaÃ§o atÃ© Ã  resistÃªncia",
    };
  }
  ...
  if (numericValue < 0.75) {
    return {
      ...
      situation:
        kind === "support"
          ? "DistÃ¢ncia curta atÃ© ao suporte"
          : "DistÃ¢ncia curta atÃ© Ã  resistÃªncia",
    };
  }
  ...
  return {
    ...
    situation:
      kind === "support"
        ? "HÃ¡ espaÃ§o razoÃ¡vel atÃ© ao suporte"
        : "HÃ¡ espaÃ§o razoÃ¡vel atÃ© Ã  resistÃªncia",
  };
}
...
function getSimpleTriggerCandleContext(
  ...
  if (candleTypeRaw.includes("strong") || candleTypeRaw.includes("trend")) {
    return {
      label: "Bom",
      situation: "Candle favorÃ¡vel ao movimento",
    };
  }
...
function getSimpleBollingerContext(
  ...
  if (isLong) {
    return {
      ...
      situation: "Reentrada favorÃ¡vel para alta",
    };
  }
  ...
  if (isShort) {
    return {
      ...
      situation: "Reentrada favorÃ¡vel para queda",
    };
  }
  ...
  if (closePositionValue !== null) {
    if (closePositionValue > 0.8) {
      return {
        ...
        situation: "PreÃ§o perto do topo da banda",
      };
    }
    ...
    if (closePositionValue < 0.2) {
      return {
        ...
        situation: "PreÃ§o perto da base da banda",
      };
    }

    return {
      label: "Neutro",
      situation: "PreÃ§o numa zona equilibrada da banda",
    };
  }
...
function buildBbReversalConfirmation(params: {
  ...
  if (candleVsAtrInfo.label === "Forte") {
    addFactor(1, 1);
  } else if (candleVsAtrInfo.label === "SaudÃ¡vel") {
    addFactor(1, 0.8);
  } else {
    addFactor(1, 0);
  }
...
function getUsefulSpaceToTargetText(
  ...
  if (sideLabel === "Compra") {
    if (isNearZone(resistanceDistanceInfo.label)) {
      return "Curto / ResistÃªncia muito perto";
    }
    return "Bom / HÃ¡ espaÃ§o atÃ© Ã  resistÃªncia";
  }
  ...
  if (sideLabel === "Venda") {
    ...
    return "Bom / HÃ¡ espaÃ§o atÃ© ao suporte";
  }
...
function getEma20NotTooAgainstText(
  ...
  if (sideLabel === "Compra") {
    if (
      ...
      return "Sim / EMA 20 nÃ£o estÃ¡ muito contra";
    }
    ...
    return "NÃ£o / EMA 20 ainda contra";
  }
...
function getNextCandleConfirmationText(item: OutcomeLikeCase): string {
  ...
  if (typeof rawValue === "boolean") {
    return rawValue ? "Sim / Candle seguinte confirmou" : "NÃ£o / Candle seguinte falhou";
  }

  if (numericValue !== null) {
    if (numericValue > 0) return "Sim / Candle seguinte confirmou";
    return "NÃ£o / Candle seguinte falhou";
  }
...
function getVolumeIndicatorText(item: OutcomeLikeCase): string {
  ...
  if (raw.includes("normal") || raw.includes("medium") || raw.includes("mÃ©dio")) {
...
function getVwapIndicatorText(
  ...
  if (sideLabel === "Compra") {
    if (raw.includes("above") || raw.includes("acima") || raw.includes("recovered")) {
      return "FavorÃ¡vel / PreÃ§o recuperou VWAP";
    }

    if (raw.includes("below") || raw.includes("abaixo")) {
      return "Contra / PreÃ§o abaixo da VWAP";
    }
  }

  if (sideLabel === "Venda") {
    if (raw.includes("below") || raw.includes("abaixo") || raw.includes("lost")) {
      return "FavorÃ¡vel / PreÃ§o perdeu VWAP";
    }

    if (raw.includes("above") || raw.includes("acima")) {
      return "Contra / PreÃ§o acima da VWAP";
    }
  }
...
function getAdxIndicatorText(item: OutcomeLikeCase): string {
  ...
  if (numericValue !== null) {
    if (numericValue >= 30) {
      return `${formatDecimal(numericValue)} / TendÃªncia forte`;
    }
    if (numericValue >= 20) {
      return `${formatDecimal(numericValue)} / TendÃªncia moderada`;
    }
    return `${formatDecimal(numericValue)} / TendÃªncia fraca`;
  }
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {sectionTitle("DecisÃ£o rÃ¡pida", rightMetrics)}
                </div>
                ...
                <div style={{ padding: 14 }}>
                  ...
                  {sectionTitle("LocalizaÃ§Ã£o do preÃ§o")}
                  <div style={{ display: "grid", gap: 6 }}>
                    {infoRow(
                      "Suporte:",
                      buildCombinedValue(
                  ...
                  {sectionTitle("Filtros prioritÃ¡rios")}
                  <div style={{ display: "grid", gap: 6 }}>
                    {infoRow("EspaÃ§o Ãºtil atÃ© ao alvo:", usefulSpaceText)}
                    {infoRow(
                    ...
                    {infoRow("ATR nÃ£o muito alta:", atrNotTooHighText)}
                    {infoRow("EMA 20 nÃ£o muito contra:", ema20NotAgainstText)}
                    {infoRow(
                      "ConfirmaÃ§Ã£o no candle seguinte:",
                  ...
                  <div style={{ display: "grid", gap: 6 }}>
                    ...
                    {infoRow("DistÃ¢ncia da banda / z-score:", bandDistanceText)}
                    {infoRow("ADX:", adxText)}
                  </div>
                  ...
                  <div style={{ display: "grid", gap: 6 }}>
                    ...
                    {infoRow("SituaÃ§Ã£o IFR:", ifrInfo.situation)}
                    {infoRow(
                      "ATR:",
                      buildCombinedValue(atrInfo.formattedValue, atrInfo.volatility),
                    )}
                    {infoRow("SituaÃ§Ã£o ATR:", atrInfo.situation)}
                  </div>
                </div>
              </div>
            );
  ...
  return (
    ...
    >
      <button
        ...
        aria-label={
          expanded
            ? "Retrair AnÃ¡lise do run selecionado"
            : "Expandir AnÃ¡lise do run selecionado"
        }
      >
        <div
          style={{
            ...
            {!selectedRunId
              ...
              : loadingRunDetails
                ...
                : runDetailsError
                  ...
                  : runDetails
                    ? `${runDetails.run.strategy_key ?? "sem strategy_key"} â€¢ ${runDetails.run.symbol} â€¢ ${runDetails.run.timeframe}`
                    : "Sem dados do run."}
          </span>
        </div>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      {formatDateTime(runDetails.run.start_at)} â†’{" "}
